...
export default class DrumSampler extends WebAudioModule<DrumSamplerNode> {
  ...
  async createAudioNode(initialState: any) {
    ...
    const paramsConfig = Object.assign(
      {},
      ...node.kit.voices.map((v, i) => v.paramsConfig(i + 1))
    );
    ...
    // Add polyphonic mode parameters
    paramsConfig["mode"] = {
      minValue: 0,
      maxValue: 1,
      defaultValue: 0, // 0 = drum mode, 1 = polyphonic mode
    };
    paramsConfig["polyphonicSampleIndex"] = {
      minValue: 0,
      maxValue: 15, // 16 slots
      defaultValue: 0,
    };
    paramsConfig["polyphonicStartNote"] = {
      minValue: 0,
      maxValue: 127,
      defaultValue: 60, // C3
    };
    const internalParamsConfig = Object.assign(
      {},
      ...node.kit.voices.map((v, i) => v.internalParamsConfig(i + 1))
    );
    ...
    const paramsMapping = Object.assign(
      {},
      ...node.kit.voices.map((v, i) => v.paramsMapping(i + 1))
    );
