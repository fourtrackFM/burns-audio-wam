...
export default class PianoRollModule extends WebAudioModule<PianoRollNode> {
  ...
  async createAudioNode(initialState: any) {
    ...
    this.sequencer.pianoRoll.updateProcessor = (c: Clip) => {
      this.sequencer.port.postMessage({
        action: "clip",
        id: c.state.id,
        state: c.getState(),
      });
    ...
    this.sequencer.pianoRoll.updateProcessorMIDIConfig = (
      config: MIDIConfiguration
    ) => {
      this.sequencer.port.postMessage({ action: "midiConfig", config });
    };

    this.sequencer.pianoRoll.sendMessageToProcessor = (message: any) => {
    ...
    this.sequencer.port.addEventListener("message", (ev) => {
      if (ev.data.event == "transport") {
        ...
        const clip = this.sequencer.pianoRoll.getClip(
          this.sequencer.pianoRoll.playingClip
        );
        const note = ev.data.note;

        // TODO: optionally don't quantize and just use note.tick
        const quantizedTick =
          Math.round(note.tick / clip.quantize) * clip.quantize;
      ...
      } else if (ev.data.event == "clipQueued") {
        console.log(
          `Clip ${ev.data.id} queued for timestamp ${ev.data.timestamp}`
        );
      } else if (ev.data.event == "clipQueueCleared") {
        console.log("Clip queue has been cleared");
      } else if (ev.data.event == "queueStatus") {
        console.log("Current queue status:", ev.data.queue);
      } else if (ev.data.nowPlayingClipID) {
        this.sequencer.pianoRoll.playingClip = ev.data.nowPlayingClipID;
      }
    });
  ...
  async createGui(clipId?: string) {
    ...
    div.setAttribute(
      "style",
      "display: flex; flex-direction: column; height: 100%; width: 100%; max-height: 100%; max-width: 100%;"
    );
    ...
    render(
      <PianoRollView
        plugin={this}
        pianoRoll={this.sequencer.pianoRoll}
        clipId={clipId}
      ></PianoRollView>,
      shadow
    );
  ...
  updateExtensions() {
    ...
    window.WAMExtensions.patterns.setPatternDelegate(
      this.instanceId,
      patternDelegate
    );
  }
